...
import WebPortfolioItemCard from './WebPortfolioItemCard';
import { WEB_PORTFOLIO_DATA, ChevronLeftIcon, ChevronRightIcon } from '../constants';
...
const WebPortfolioSection: React.FC<WebPortfolioSectionProps> = ({ id, showWebPortfolioView }) => {
  ...
  const totalProjects = WEB_PORTFOLIO_DATA.length;
  ...
  if (totalProjects === 0) {
    return (
      <div className="py-16 md:py-24 scroll-mt-16 bg-neutral-DEFAULT w-full">
        ...
        <div ref={contentBlockRef} className={`${contentBlockInView ? 'animate-fade-in-up' : 'opacity-0'}`}>
            <p className="text-center text-neutral-dark">No web projects to display yet.</p>
        ...
        <div ref={contentBlockRef} className={`${contentBlockInView ? 'animate-slide-in-from-right' : 'opacity-0'}`}>
          <div className="relative w-full">
              ...
              {WEB_PORTFOLIO_DATA.map((item: WebPortfolioItem) => (
                  ...
                  >
                  <WebPortfolioItemCard item={item} showEnlargeButton={false} />
                  </div>
              ))}
              </div>
